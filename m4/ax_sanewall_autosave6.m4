...
#   Output:
#
#   $SANEWALL_AUTOSAVE6 contains the path to the file, or is empty if none
...
AC_DEFUN([AX_SANEWALL_AUTOSAVE6],
[
...
!
	sh conftest-sw-autosave6 > /dev/null 2>&1
        ...
        SANEWALL_AUTOSAVE6="`cat conftest-sw-autosave6.out`"
    elif test -d /etc/iptables; then
        autosave6_system=" (Debian)"
        SANEWALL_AUTOSAVE6="/etc/iptables/rules.v6"
    else
    ...
    if test "$ac_with_autosave6" = "no"; then
            ...
            SANEWALL_AUTOSAVE6="$ac_with_autosave6";
    fi
fi
AC_MSG_RESULT([$SANEWALL_AUTOSAVE6$autosave6_system])
AC_SUBST(SANEWALL_AUTOSAVE6)
])
