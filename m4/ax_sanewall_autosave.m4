...
#   Output:
#
#   $SANEWALL_AUTOSAVE contains the path to the file, or is empty if none
...
AC_MSG_CHECKING([for default SANEWALL_AUTOSAVE])
...
!
	sh conftest-sw-autosave > /dev/null 2>&1
        ...
        SANEWALL_AUTOSAVE="`cat conftest-sw-autosave.out`"
    elif test -d /etc/iptables; then
        autosave_system=" (Debian)"
        SANEWALL_AUTOSAVE="/etc/iptables/rules.v4"
    else
    ...
    if test "$ac_with_autosave" = "no"; then
            ...
            SANEWALL_AUTOSAVE="$ac_with_autosave";
    fi
fi
AC_MSG_RESULT([$SANEWALL_AUTOSAVE$autosave_system])
AC_SUBST(SANEWALL_AUTOSAVE)
])
